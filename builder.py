...
class MakeEnv:
    ...
    def get_src(self) -> None:
        ...
        """
        subprocess.run(
            ['git', 'clone', 'https://github.com/pateu198/impulse.git'], cwd=self.build_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        shutil.move(os.path.join(self.build_dir,
                    'impulse', 'src'), self.build_dir)
...
def main() -> None:
    ...
    with progress:
        task1 = progress.add_task("[bold green]Loading...", total=1)
        make_env = MakeEnv()
        make_env.make_env()
        make_env.get_src()
        progress.update(task1, advance=1)

        task2 = progress.add_task("[bold green]Loading...", total=1)
        write_config = WriteConfig(config_data)
        write_config.write_config()
        progress.update(task2, advance=1)

        task3 = progress.add_task("[bold green]Loading...", total=1)
        do_obfuscate = DoObfuscate()
        do_obfuscate.run()
        progress.update(task3, advance=1)

        task4 = progress.add_task("[bold green]Loading...", total=1)
        build = Build()
